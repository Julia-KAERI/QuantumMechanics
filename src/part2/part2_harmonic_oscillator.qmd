...
시간독립 슈뢰딩거 방정식 $H|\Psi\rangle = E|\Psi\rangle$ 을 $X$ 기저에서 표현하면 아래와 같다.

$$
\dfrac{d^2 \Psi(x)}{dx^2} + \dfrac{2m}{\hbar^2}\left(E-\dfrac{1}{2}m\omega^2 x^2\right)\Psi(x)=0
$$ {#eq-part2_1dsho_time_independent_schroedinger_eq}


여기에 $b=\left(\dfrac{\hbar}{m\omega}\right)$ 에 대해 $y=x/b$ 로 치환한다. $\varepsilon = \dfrac{E}{\hbar \omega}$ 로 놓으면 @eq-part2_1dsho_time_independent_schroedinger_eq 은 다음과 같이 좀 더 단순한 방정식으로 변한다. 

$$
\dfrac{d^2\Psi(y)}{dy^2}+ (2\varepsilon - y^2)\Psi(y) = 0
$$ {#eq-part2_substituted_shroedinger_eq}

$y\to\infty$ 극한에서 $2\varepsilon - y^2 \to -y^2$ 가 된다. 즉 $|y|$ 가 매우 큰 극한에서 $\Psi(y)$ 는
