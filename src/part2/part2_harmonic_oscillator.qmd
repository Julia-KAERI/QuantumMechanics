...
$$ {#eq-part2_sho_energy_eigenstate_with_hermite_polynomial}
...
에르미트 다항식에 대해 알아보자. 우선 @eq-part2_generator_of_hermite_polynomial 로 정의된 에르미트 다항식의 첫 4 함수를 정의대로 계산해보면 다음과 같다.(여기서는 변수를 $\xi$ 에서 $y$ 로 변경한다.)
...
H_3(y) &= -12 y + 8 y^3.
\end{aligned}
$$ {#eq-part2_sho_hermite_polynomial_first_4}

이제 에르미트 다항식에 대해 본격적으로 알아보자.  에르미트 다항식은 @eq-part2_sho_energy_eigenstate_with_hermite_polynomial 로 정의
...
@eq-part2_sho_energy_eigenstate_with_hermite_polynomial 를 생각하자. 우리는 $a|n\rangle = \sqrt{n}|n-1\rangle$ 임을 안다. 이를 이용하면
...
$$
H'_n(y) = 2nH_{n-1}(y).
$$  {#eq-part2_hermite_polynomial_recurssion_relation_1}


또한 @eq-part2_generator_of_hermite_polynomial 로부터 다음의 결과를 얻을 수 있다.
$$
\begin{aligned}
...
$$ {#eq-part2_hermite_polynomial_recurssion_relation_2}


그리고 @eq-part2_hermite_polynomial_recurssion_relation_1 과 @eq-part2_hermite_polynomial_recurssion_relation_2 로 부터 다음의 점화식을 얻을 수 있다.

$$
H_{n+1}(y) = 2yH_n(y) - 2nH_{n-1}(y).
$$ {#eq-part2_hermite_polynomial_recurssion_relation_3}
...
$$ {#eq-part2_orthogonality_of_hermite_polynomials}
...
@eq-part2_sho_energy_eigenstate_with_hermite_polynomial 에서 우리는 $H_n(x)$ 를 구하는 방법을 알아보았다. 이제 이것을 에너지 고유상태에 대해 그래프를 그려보면 아래와 같다.

![1차원 단순조화진동자의 파동함수](figure/fig-part2_1d_sho_wavefunction.png){#fig-part2_sho_wavefunctions width=500}
...
$x=0$ 에서의 경계조건은 $\Psi(0) = 0$ 이다. @eq-part2_sho_hermite_polynomial_first_4 과 @eq-part2_hermite_polynomial_recurssion_relation_3 로부터 에르미트 다항식의 유용한 성질을 얻을 수 있다. $H_0(y)$ 부터 $H_3(y)$ 까지 보면 $H_n(y)$ 에서 $n$ 이 짝수이면 짝함수(even function), $n$ 이 홀수 이면 홀함수 인 것 같다. 이제 @eq-part2_hermite_polynomial_recurssion_relation_3 를 이용하여 보이자. $n$ 보다 작거나 같은 정수에 대해 이 명제가 성립한다고 가정하자. @eq-part2_hermite_polynomial_recurssion_relation_3 로부터
...
이제 $H_{n}(y)$ 의 상수항을 생각하자. $n$ 이 홀수이면 홀함수 이므로 상수항이 $0$ 이다. 역시 @eq-part2_hermite_polynomial_recurssion_relation_3 에서
