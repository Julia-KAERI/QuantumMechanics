...
### 자유 입자 문제의 해와 시간 전개 {#sec-part2_free_particle_and_time_evolution}

가장 간단한 자유입자의 경우 슈레딩거 방정식은 다음과 같다.
...
이며 이로부터 $p = \pm \sqrt{2mE}$ 를 얻는다. 즉 각각의 $|E\rangle$ 상태는 $|p=+\sqrt{2mE}\rangle$ 상태와 $|p=-\sqrt{2mE}\rangle$ 상태의 선형결합으로 표현 할 수 있다. 이렇게 해밀토니안 $H$ 와 commute 한 연산자 $A$ 에 대해 $H$ 의 어떤 고유상태가 $A$ 의 서로 다른 고유상태가 섞여 있을 수 있으며 이것을 **축퇴 상태(degenerated state)** 라고 한다. 자유입자의 경우 $H$ 의 고유상태 $|E\rangle$ 은 운동량 연산지 $P$ 의 두 고유상태 $|p=\sqrt{2mE}\rangle$ 과 $|p=-\sqrt{2mE}\rangle$ 가 섞여 있으며, 따라서 아래와 같이 
...
$$
U(t) = \int_{-\infty}^\infty |p\rangle \langle p| e^{-ip^2/2m\hbar}\, dp 
$$ {#eq-time_evolution_of_free_particle}

이며 이것을 $X$ 기저로 표현하면

$$
\begin{aligned}
U(x,\,t; x',\,0) := \langle x|U(t)|x'\rangle &= \int_{-\infty}^\infty \langle x|p\rangle \langle p|x'\rangle e^{-2p^2/2m\hbar}\, dp \\
&= \dfrac{1}{2\pi \hbar} \int_{-\infty}^\infty e^{ip(x-x')} e^{-2p^2/2m\hbar}\, dp \\
&= \left(\dfrac{m}{2\pi \hbar i t}\right)^{1/2} e^{im(x-x')^2/2\hbar t}
\end{aligned}
$$ {#eq-part2_time_evolution_of_operator_for_free_particle_in_X_basis}

이다. 따라서 $t=0$ 일 때 $\psi(x',\,t=0)$ 인 상태였다면

$$
\psi(x,\,t) = \langle x|\psi(t)\rangle = \int\langle x|U(t)|x'\rangle \langle x'|\psi(0)\rangle \, dx' = \int U(x,t;x',0) \, \psi(x',\,0)\, dx'
$$
...
이다. @eq-part2_time_evolution_of_operator_for_free_particle_in_X_basis 를 통해 알 수 있듯이 $t=0$ 일 때 국소화 되어 있던 입자는 시간이 지날수록 공간상으로 넓게 퍼지게 된다. 
...
$${#eq-part2_gaussian_wave_packet_with_definite_momentum}
...
\end{aligned}
$$
@eq-part2_time_evolution_of_operator_for_free_particle_in_X_basis 와 @eq-part2_gaussian_wave_packet_with_definite_momentum 를 통해
...
::: {#exr-part2_gaussian_wave_packet}

@eq-part2_gaussian_wave_packet_with_definite_momentum 로 주어진 웨이브 페킷에 대해 다음을 보여라.
...
@exr-part2_gaussian_wave_packet 의 ($1$) 을 보자. 우리는 고전역학에서 $t=0$ 일 때 $x=0$ 인 자유입자는 $x=pt/m$ 를 따른다는 것을 안다. 양자역학에서는 통계적으로 $X$ 의 기대값 $\langle X\rangle$ 과 $P$ 의 기대값 $\langle P\rangle$ 사이에 $\langle X\rangle = \langle P\rangle t/m$ 의 관계를 갖는데 이것은 **에렌페스트 정리(Ehrenfest theorem)** 의 결과중 하나이다. 

@exr-part2_gaussian_wave_packet 의 ($2$) 는 웨이프 패킷의 폭, 즉 위치에 대한 불확정성이 시간에 따라 점점 증가함을 보여준다. 다음의 예를 보자
...
::: {#exr-part2_shankar_5_1_3}

#### 가우시안 패킷 문제에 대한 다른 접근

자유입자에 대해 $U(t) = e^{-iHt/\hbar}$ 이며 $p = -i\hbar \dfrac{d}{dx}$ 이므로

$$
U(t) = \exp \left[\dfrac{i}{\hbar} \left(\dfrac{\hbar^2 t}{2m} \dfrac{d^2}{dx^2}\right)\right] = \sum_{k=0}^\infty \dfrac{1}{k!} \left(\dfrac{i\hbar t}{2m}\right)^k\dfrac{d^{2k}}{dx^{2k}}
$$ {#eq-part2_another_form_of_gaussian_packet}

로 쓸 수 있다. 초기 상태를 @eq-part2_gaussian_wave_packet_with_definite_momentum 에서 $p_0=0$, $\Delta =1$ 로 잡으면

$$
\psi(x,\,0) = \dfrac{e^{-x^2/2}}{\pi^{1/4}}
$$

이다. @eq-part2_another_form_of_gaussian_packet 를 이용하여 $\psi(x,\,t)$ 를 구하고 @eq-gaussian_wavepacket_1 와 비교하라
...
$$
\psi(x,t) = U(t)\,\psi(x, 0) = \dfrac{1}{\pi^{1/4}}\sum_{k=0}^{\infty} \sum_{j=0}^\infty \left(\dfrac{i\hbar t}{2m}\right)^k \dfrac{1}{k!} \dfrac{d^{2k}}{dx^{2k}} \left(\dfrac{(-1)^j x^{2j}}{j! (2^j)} \right) \tag{2}
$$
...
### 무한 포텐셜 우물 {#sec-part2_infinite_potential_well}

![(좌) 무한 포텐셜 우물, (우) 가장 낮은 두 파동함수](figure/fig-part2_infinite_potential_well.png){#fig-part2_infinite_potential_well width=600}


포텐셜 $V(x)$ 가 다음과 같이 주어졌다고 하자. (@fig-part2_infinite_potential_well)

$$
V(x) = \left\{\begin{array}{ll}  0, \qquad & |x|<L/2 \\ \infty, & |x|\ge L/2 \end{array} \right.
$$
...
$$ {#eq-part2_particle_in_box_1}

구간을 $V(x)$ 의 특성에 따라 @fig-part2_infinite_potential_well 과 같이 $\rm{I},\, \rm{II},\, \rm{III}$ 으로 나누자. $\rm{I},\, \rm{III}$ 구간에서의 해 $\psi_\infty(x)$를 우선 구해보자. $V_0 >E$ 에 대해
...
이제 $\rm{II}$ 구간의 파동함수 $\psi_{\rm{II}(x)}$ 를 생각해보자. $k = \sqrt{2mE/\hbar^2}$ 라고 하면 @eq-part2_particle_in_box_1 의 해는
...
$$ {#eq-part2_wavefunction_of_infinite_potential_wall}
...
$$ {#eq-part2_energy_eigenvalues_of_infinite_potentail_wall}

이다. 즉 박스 안의 입자가 가질 수 있는 에너지는 불연속적이다. 이렇게 어떤 동역학적인 값에 대해 불연속적인 값만 허용될 때 이를 **양자화(quantization)** 이라고 한다. 또한 파동함수는 $|x|<L/2$ 영역에서만 $0$ 이 아닌 값을 갖는다. 이렇게 파동함수 $\psi(x)$ 가 

$$
\lim_{|x| \to \infty} \psi (x) = 0
$$

일 때 이 상태를 **bound state** 라고 한다. 고전적인 bound state 는 $V(x)$ 가 $E$ 보다 작은 영역이 국소화 되어 있을 때 발생하며, 양자역학도 마찬가지다. 더군다나 양자역학에서는 bound state 의 에너지는 필연적으로 양자화된다. 

</br>

### 포텐셜 우물 {#sec-part2_potential_well}

![포텐셜 우물](figure/fig-part2_potential_well.png){#fig-potential_well width=300}
...
$0<E<V_0$ 인 경우를 생각해보자 이 경우 $\rm{I}$ 과 $\rm{III}$ 은 고전적으로 금지된 영역이다. 그러나 무한 포텐셜 우물에서와는 다르게 $\rm{I}$ 과 $\rm{III}$ 의 파동함수는 $0$ 이 아니며 $\kappa = \sqrt{2m(V_0-E)/\hbar^2}$, $k=\sqrt{2mE}/\hbar^2$ 에 대해 

$$
\begin{aligned}
\psi_\rm{I}(x) &= A_1 e^{\kappa x}, \\
\psi_{\rm{II}}(x) &= A\cos (k x) + B \sin (k x), \\
\psi_\rm{III}(x) &= A_3 e^{-\kappa x}.
\end{aligned}
$$

이다. 이제 파동함수와 그 도함수의 연속 조건으로부터
...
![($a$) 짝함수의 경우에서의 허용된 $\kappa$ 값 ($b$) 홀함수의 경우 허용된 $\kappa$ 값. 스케일을 위해 가로축을 $ka$ 로, 세로축을 $\kappa a$ 로 잡았다. ](figure/fig-part2_solution_of_potential_well.png){#fig-part2_energy_qunatization_of_potential_well width=800}
...
### Bound state 와 에너지 양자화 {#sec-part2_bound_state_and_energy_quantization}


앞서의 무한포텐셜 우물과 포텐셜 우물의 경우 상태가 bound state 일 경우 에너지가 양자화된다. 이것은 일반적으로 bound state 에서 성립하는데 여기서는 이것을 보이고자 한다. 정해진 $E$ 에 대해 $V(x)$ 는 다음의 성질을 가진다고 가정하자 하자.
...
양 끝단을 기술하는 파동함수 $\psi_1$ 과 $\psi_N$ 은 각각 하나의 미지수 $A_1,\,A_N$ 를 가진다. 그 외의 구간에서는 두개의 미지수 $A_i$ 와 $B_i$ 를 가진다. 이 미지수는 파동함수 $\psi_i(x)$ 와 그 미분인 $\psi'_i(x)$ 가 인접한 $\psi_{i-1}(x),\, \psi'_{i-1}(x)$ 그리고 $\psi_{i+1}(x),\, \psi'_{i+1}(x)$ 와 연속이어야 한다는 조건으로부터 결정 될 수 있다. 즉 우리는 $2\times (N-2)+ 2 = 2N-2$ 개의 미지수에 대해 $2N-2$ 개의 경계조건을 가진다. 그런데 여기에서 주의해야 할 것이 있다. 미지수에 대한 식은 미지수 전체에 $0$ 이 아닌 수를 곱해도 성립한다. 즉 $A_i,\,B_i$ 에 대해 중요한것은 각각의 비율이다. 여기에서 미지수의 자유도를 제한하는 조건이 하나 들어가므로 실제로 미지수의 자유도는 $2N-3$ 이며 이에 대해 $2N-2$ 개의 경계조건이 존재하게 된다. 이것이 에너지 양자화의 원인이다. 이 조건은 구간을 늘려도 확실히 성립한다. 
...
### 바닥상태 에너지 {#sec-part2_ground_state_energy}
...
::: {#exr-part2_potential_wall_1}

무한 포텐셜 우물의 파동함수 $\psi_n(x)$(@eq-part2_wavefunction_of_infinite_potential_wall) 에 대한 시간전개 연산자 $U(t)$ 를 구하고 이것을 $X$ 의 기저로 표현하라.
...
$$ {#eq-part2_classical_continuity_equation_of_charge}
...
$${#eq-part2_classical_continuity_equation_of_charge_2}
...
를 얻는다. @eq-part2_classical_continuity_equation_of_charge_2 와의 유사성을 생각하면 

$$
\boldsymbol{j} := \dfrac{\hbar}{2mi} (\Psi^\ast \nabla \Psi - \Psi \nabla \Psi^\ast)
$$ {#eq-part2_definition_of_probability_current_density}

에 대해

$$
\dfrac{\partial P}{\partial t} = \nabla \cdot \boldsymbol{j}
$$ {#eq-part2_continuity_equation_of_probability}

를 얻게 된다. 이 때 $\boldsymbol{j}$ **확률 흐름 밀도(probability current density)** 라고 하고 @eq-part2_continuity_equation_of_probability 를 **연속 방정식(continuity equation)** 이라고 한다.
...
이 때 $\boldsymbol{j}$ 와 $P$ 를 구하고 전자기 방정식 $\boldsymbol{j}= \rho \boldsymbol{v}$ 와 비교하라. 여기서 $\boldsymbol{v}$ 는 속도이다. $\rho$ 와 $\boldsymbol{j}$ 는 상수이며 따라서 @eq-part2_continuity_equation_of_probability 가 자동작으로 만족된다.
...
::: {#thm-part2_no_degeneracy_in_1d_bound_state}
...
::: {#thm-part2_eigenfunction_of_H_is_pure_real_function_in_X_basis}
