...
을 만족해야 한다. $\rho\to 0$ 인 극한에서 $R(\rho) \approx \rho^k$ 라고 하자. 파동함수는 연속이어야 하므로 $R(\rho)$ 는 원점에서 finite 해야 하며 따라서 $k>0$ 이어야 한다. $R(\rho) \approx \rho^k$ 에 대해 @eq-part2_shankar_12_3_7_3 은
...
($2$) $\rho \to \infty$ 극한에서 $R(\rho \to \infty) = 0$ 이어야 하며, 위의 미분방정식 @eq-part2_shankar_12_3_7_3 은
...
$$ {#eq-part2_shankar_12_3_7_5}

라고 놓을 수 있다. 이 식을 @eq-part2_shankar_12_3_7_4 의 조변에 넣으면

$$
\dfrac{\partial^2 R}{\partial \rho^2} - \dfrac{\mu^2 \omega^2}{\hbar^2} \rho^2 R(\rho) - \dfrac{\mu \omega}{\hbar} R(\rho) 
$$

이 되는데 앞서 보았듯이 $\rho \to \infty$ 극한에서 $|\rho^2 R(\rho) | \gg |R(\rho)|$ 이므로 근사적으로 @eq-part2_shankar_12_3_7_4 를 만족한다고 볼 수 있다.
