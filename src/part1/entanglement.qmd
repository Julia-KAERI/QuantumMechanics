...
::: {.proof}
우리가 관심 있는 측정량 $L$ 의 고유벡터를 기저 $\{|\phi_i\rangle \}$ 로 삼으면 $|\phi \rangle = \sum_i a_i |\phi_i\rangle$ 에 대해

$$
\langle L \rangle = \langle \phi |L |\phi \rangle = \sum_{i} |a_i|^2 \langle \phi_i|L|\phi_i\rangle 
$$
...
$$
\rho_{ii'} = \sum_j c_{ij}\overline{c_{i'j}} = \sum_j \langle a_ib_j |\mathscr{\Psi}\rangle \langle \mathscr{\Psi} | a_{i'}b_j\rangle
...
@eq-entanglement2_density 은 순수상태의 밀도행렬이 아닌 혼합상태의 밀도행렬이다. 우리가 @eq-entalgnement2_wavefunction 에서의 계수 $c_{ij}$ 를 완벽하게 알고 있다고 하자. 그렇더라도 밀도행렬 @eq-entanglement2_density 는 순수상태의 밀도행렬이 아닌 혼함상태의 밀도형렬이다. 즉 전체계가 순수상태더라도 그 부분계는 복합상태로 기술되어야 한다.
