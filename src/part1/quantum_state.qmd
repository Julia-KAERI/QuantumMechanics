...
## 양자 역학의 원리 {#sec-principles_of_quantum_physics}
...
**원리 2.** 양자역학의 측정량은 에르미트 연산자이다$^1$. 즉 $\hat{A}^\dagger = \hat{A}$ . [$^1$ 관례적으로 양자역학에서 연산자를 $\hat{A}$ 처럼 문제 위에 $\hat{\,}$ 기호를 씌워 표기하는 경우가 많다. 물론 생략해도 상관 없다.]{.aside}
...
$$
\forall\, |\phi\rangle,\,|\psi\rangle \in V,\qquad \langle \phi | \hat{A}^\dagger | \psi \rangle = \langle \phi |\hat{A}|\psi \rangle. 
...
$$
I = U^\dagger(\varepsilon) U(\varepsilon) = (I +i\varepsilon \hat{K}^\dagger)(I - i \varepsilon \hat{K}) = I -i\varepsilon (\hat{K}-\hat{K}^\dagger) + O(\varepsilon^2)
$$
...
두 연산자 $\hat{A},\, \hat{B}$ 에 대해 $[\hat{A},\, \hat{B}] := \hat{A}\hat{B} - \hat{B}\hat{A}$ 를 $\hat{A}$ 와 $\hat{B}$ 에 대한 **교환자(commutator)** 라고 한다. 


:::
:::

이를 이용하면 @eq-p1_commutator_1 으로부터 다음을 얻는다.
...
이다. 이제 $\hat{A},\,\hat{B}$ 를 각각 $\hat{A}-\langle \hat{A}\rangle \hat{I},\, \hat{B}-\langle \hat{B}\rangle \hat{I}$ 로 바꾸자. 그렇다면 @eq-p1_uncertainty-3 의 우변은
