...
어떤 두 측정은 서로 완전히 독립적이라 하나의 측정이 다른 하나의 측정에 영향을 끼치지 않을 수 있다. 그러나 어떤 두 측정은 하나의 측정이 다른 하나의 측정에 영향을 준다. $L$ 과 $M$ 의 두 측정, 혹은 두 에르미트 연산자를 생각해보자. 우리는 @prp-part1_uncertainty_1 에서 $[L,\,M]=0$ 인 것과 $L$ 과 $M$ 에 대한 동시 고유벡터로 이루어진 기저가 존재한다는 것이 수학적으로 동치라는 것을 보였다. 이 때 $[L,\, M]=0$ 인 것을 두 연산자 $L,\, M$ 이 **교환 가능하다**라고 한다.
