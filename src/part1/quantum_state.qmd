...
@eq-part1_quantum_state_2, @eq-part1_quantum_state_3 로부터 $\hat{\sigma}_x$ 의 교유벡터 $|r\rangle,\, |l\rangle$ 에 대한 행렬 표현을 얻을 수 있다.
...
이다. 또한 @eq-part1_quantum_state_5 로 부터 $\hat{\sigma}_y$ 의 고유벡터 $|i\rangle,\, |o\rangle$ 에 대한 행렬표현을 얻을 수 있다.
...
$$ {#eq-part1_definition_of_hermite_conjugate}

이 때 $\hat{A}^\dagger$ 역시 벡터공간 $V$ 에서 정의된 선형연산자이다. **에르미트 연산자(Hermitian operator)** 는 다음을 만족하는 연산자로 @sec-part1_hermitian_operator 를 참고하라.

$$
\forall\, |\phi\rangle,\,|\psi\rangle \in V,\qquad \langle \phi | \hat{A}^\dagger | \psi \rangle = \langle \phi |\hat{A}|\psi \rangle. 
$$ {#eq-part1_definition_of_hermite_operator}
...
을 얻는다. 이제 $\hat{K}$ 에 대해 생각해보자. $\hat{K}$ 는 시간의 역수의 차원을 가진다. 이제 플랑크 상수 $\hbar = \dfrac{h}{2\pi} = 6.62607015 \times 10^{-34}\, \text{m}^2 \,\text{kg} / \text{s}$ 에 대해 $\hat{H} = \hbar \hat{K}$ 라고 놓으면 $\hat{H}$ 는 에너지와 같은 차원을 가지는 에르미트 연산자임을 안다. $\hat{K}$ 가 에르미트 연산자이므로 $\hat{H} = \hbar \hat{K}$ 가 에르미트 연산자임은 당연하다. @eq-part1_hamiltonian_1 으로부터 
...
을 얻는다. @eq-part1_time_dependent_schroedinger_equation 을 **시간 의존 슈뢰딩거 방정식 (time-dependent Schrödinger equation)** 이라고 한다. 또한 $\hat{H}$ 를 **해밀토니안(Hamiltonian)** 이라고 한다. $\hat{H}$ 가 해밀토니안이라는 고전역학에서 에너지에 대한 물리량과 같은 이름을 갖게 되는지는 좀 더 진행해 봐야 한다. 이를 위해서 잠시 길을 돌아가기로 하자.
...
이다. 이제 $\hat{A},\,\hat{B}$ 를 각각 $\hat{A}-\langle \hat{A}\rangle \hat{I},\, \hat{B}-\langle \hat{B}\rangle \hat{I}$ 로 바꾸자. 그렇다면 @eq-part1_uncertainty-3 의 우변은
