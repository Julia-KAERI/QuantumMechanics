...
힐베르트 공간은 벡터공간이며 따라서 힐베르트 공간의 원소는 벡터이다. 역사적, 실용적 이유로 양자역학에서의 표기법은 보통 수학에서의 표기법과는 다른 브라켓 표기법을 사용하며 대표적으로 벡터는 $|a\rangle$ 과 같이 $|\,\rangle$ 형태로 표기하며, 선형연산자는 $\hat{L}$ 과 같이 문자 위에 $\hat{~}$ 기호를 씌워 표기한다. 양자역학에서 중요한 상태는 수학적으로 힐베르트 공간의 벡터이다. 

</br>

## 브라켓 표기법 {#sec-braket_notation}

브라켓 표기법은 양자역학에서 벡터와 선형범함수를 표기하는 표기법으로 폴 디랙에 의해 고안되었다. 양자역학은 유한차원 복소내적벡터공간, 혹은 무한차원 복소내적벡터공간인 힐베르트 공간에서 기술되며 역사적인 이유로 고유의 표기법인 브라켓 표기법을 사용하게 되었다. 
