...
힐베르트 공간 $\mathcal{H}$ 에서 정의된 선형사상 $A:\mathcal{H} \to \mathcal{H}$ 를 **연산자(operator)** 라고 한다. 선형사상은 임의의 $|a\rangle,\, |b\rangle,\, c\in \mathbb{F}$ 에 대해
...
$3$ 의 성질을 **completness** 라고 부른다. $\mathcal{H}$ 의 에르미트 연산자 $A$ 의 고유벡터로 이루어진 정규직교기저 $\{|a_1\rangle ,\, |a_2,\rangle,\ldots\}$ 를 생각하자. 임의의 $|\psi\rangle \in \mathcal{H}$ 에 대해 
...
는 고유값 $1$ 과 $2$ 를 갖는 에르미트 연산자이며 $\boldsymbol{e}_1$ 과 $\boldsymbol{e}_2$ 는 같은 고유값 $1$ 을 갖는 $A$ 의 고유벡터이다. 이렇게 같은 고유값을 갖는 선형 독립인 고유벡터가 있을 경우를 **degeneracy** 라고 한다. 우리는 degeneracy 가 있는 연산자에 대해 같은 고유값을 갖는 벡터의 집합은 $\mathcal{H}$ 의 부분공간을 이룬다는 것을 안다. 이것을 고유공간(eigenspace) 라고 하며 $\mathcal{H}$ 의 고유공간에 대해 정규직교기저를 구성할 수있다는 것을 안다. 

즉 $\mathcal{H}$ 에서 정의된 에르미트 연산자 $A$ 의 고유값이 $\lambda_1,\,\lambda_2,\ldots$ 일 때 $\lambda_i$ 에 대한 고유공간의 차원이 $1$ 이면 정규벡터 $|a_i\rangle$ 을 찾을 수 있으며, $\lambda_j$ 에 대한 고유공간이 2차원 이상일 때 이 고유공간의 정규직교기저 $\left| a_j^{(1)}\right\rangle,\, \left|a_j^{(2)}\right\rangle,\ldots$ 를 찾을 수 있다. 이 때, 

$$
\langle a_i^{(k)}|a_j^{(l)}\rangle = \delta_{ij}\delta_{kl}
$$

이 된다. 이렇게 이루어진 정규직교벡터의 집합 $\{a_i^{(k)}\}$ 는 $\mathcal{H}$ 의 정규직교기저를 이룬다.
...
힐베르트 공간 $\mathcal{H}$ 에서 정의된 두 에르미트 연산자 $L,\,M$ 에 대해 다음은 동치이다.

&emsp; ($1$) $[L,\,M]= 0$.

&emsp; ($2$) $L$ 과 $M$ 에 대해 동시에 모두 고유벡터인 벡터로 이루어진 $\mathcal{H}$ 의 기저가 존재한다.
:::

::: {.proof}

($1 \implies 2$) $L$ 과 $M$ 이 에르미트 연산자이므로 각각의 고유상태로 이루어진 $\mathcal{H}$ 의 기저가 존재한다. 고유값 $l_i$ 인 $L$ 의 고유상태를 $|l_i\rangle$ 로 표기하자. 
