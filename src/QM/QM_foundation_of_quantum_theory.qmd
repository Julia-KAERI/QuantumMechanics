...
우리는 1부에서 비상대론적 양자역학의 4가지 원리와 슈레딩거 방정식에 대해 알아보았다. 이것을 여기에 다시 기술해보자.
...
**정리 1** 의 분리가능한 힐베르트 공간은 항상 가산무한개의 기저를 가질 수 있다. 즉 우리는 기저를 $|\phi_1\rangle,\, |\phi_2\rangle, \ldots$ 과 같이 표현 할 수 있다는 의미이다. 양자역학의 원리를 분리가능한 힐베르트 공간에서 구성한 것은 폰 노이만이다. 앞으로는 힐베르트 공간이라고 할 때는 분리가능한 힐베르트 공간을 의미한다.
...
이 특성의 증명은 연습문제(@exr-QM_properties_of_projection_operator) 로 넘긴다.
...
**원리 2** 에서 우리는 고전적인 측정값 $\omega (x,\,p)$ 가 양자 역학에서는 연산자 $\Omega(X,\,P)$ 가 된다고 했다. 그렇다면 $omega = xp$ 는? $XP$ 인가 아니면 $PX$ 인가? 이 경우는 조작하여 대칭이 되도록 한다. 즉 $\Omega = \dfrac{1}{2}(XP+PX)$ 로 정한다. 이 경우 $XP$ 도 $PX$ 도 에르미트 연산자가 아니지만 $\dfrac{1}{2}(XP+PX)$ 는 에르미트 연산자가 된다. 하지만 $\omega = x^3p^2$ 와 같은 경우라면 어떨까? 
...
::: {#exr-QM_properties_of_projection_operator}

#### 사영연산자의 성질

힐베르트 공간 $\mathcal{H}$ 에서 정의된 사영연산자 $P_\omega = |\omega \rangle \langle \omega|$ 는 다음 성질을 가짐을 보여라.
