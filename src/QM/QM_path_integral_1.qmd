...
이다. 우리는 시간전개 연산자 $\mathcal{U}(x,\,t;x,\,t')$ 에 대해
...
\begin{aligned}
\Psi&(x,\,\varepsilon)  = \int_{-\infty}^\infty \mathcal{U}(x,\,\varepsilon;x') \Psi(x,\, 0)\, dx'\\
&=\left(\dfrac{m}{2i \pi \hbar \varepsilon}\right)^{1/2} \int_{-\infty}^{\infty}\exp \left[\dfrac{i m (x-x')^2}{2\varepsilon\hbar}\right] \exp \left[- \dfrac{i\varepsilon}{\hbar} V\left(\dfrac{x+x'}{2},\,0\right)\right] \, \Psi (x',\,0)\, dx'
\end{aligned}
$$

이다. 여기서 $\exp \left[\dfrac{i m (x-x')^2}{2\varepsilon\hbar}\right]$ 을 보자. $\varepsilon$ 은 infinitesimal number 이며 $\hbar$ 역시 매우 작은 수이다. 그러나 $\exp \left[- \dfrac{i\varepsilon}{\hbar} V\left(\dfrac{x+x'}{2},\,0\right)\right]$ 는 앞서의 항에 비해 매우 느리게 변하는 함수이다. 