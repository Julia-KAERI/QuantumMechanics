...
우리는 시간전개 연산자 $\mathcal{U}=e^{iHt/\hbar}$ 를 알면 양자역학 풀린다는 것을 안다. 이를 위해 해밀토니안 연산자에 대한 고유값과 고유벡터를 구했고 시간전개 연산자를 이 고유상태에 대해 표현했다. 경로적분법은 시간전개 연산자를 직접 구하는데 그 과정은 다음과 같다. 여기서는 1차원에 대해 다루며 다차원의 경우로 쉽게 일반화 할 수 있다. 즉 $\mathcal{U}(x,\,t; x',\,t')$ 을 아래의 과정을 통해 구한다.
...
$$
\mathcal{U}(x,\,t;x',\,t') = \displaystyle A\sum_{\text{all paths}} e^{iS[x(t)]/\hbar}
$$ {#eq-QM_propergator_using_path_integral}

여기서 $A$ 는 정규화 상수이다. 혹은 $\mathcal{U}(x,\,t;x',\,t')$ 을 **Feynmann kernel** 이라고 부르며 $K(x,\,t;x',\,t')$ 이라고 쓰기도 한다.
...
&\approx \dfrac{1}{2} m \dfrac{(x-x')^2}{\varepsilon} - V\left(\dfrac{x+x'}{2},\,0\right)\varepsilon
\end{aligned}
$$

이다. 
$$
\mathcal{U}(x,\,\varepsilon;x') = 
$$
