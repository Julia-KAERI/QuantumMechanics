...
스칼라 파동함수의 경우 $\delta \bf{\theta}$ 회전은 $(x,\,y,\,z)$ 에 지정된 스칼라값을 새로운 $(x',\,y',\,z')$ 에 지정하는 것이다. 벡터장 파동함수의 경우는 ($a$) $(x,\,y,\,z)$ 에 지정된 벡터를 $\delta \bf{\theta}$ 만큼 회전시키고, ($b$) 이 값을 새로운 $(x',\,y',\,z')$ 에 할당한다. 미분연산자 $L_x,\,L_y,\,L_z$ 는 ($b$) 의 기능만을 하며 ($a$) 의 기능은 $3\times 3$ 
...
&\longrightarrow \bf{\Psi}'(x,\,y) = \left[\Psi_x(x+\varepsilon_z y,\, y-\varepsilon_z x) - \varepsilon_z  \Psi_y(x+\varepsilon_z y,\, y-\varepsilon_z x)\right] \hat{\bf{e}}_x \\[0.3em]
