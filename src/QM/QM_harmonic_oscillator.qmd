...
이제 @eq-QM_1d_sho 로 주어진 시간독립 슈뢰딩거 방정식을 풀어보자. 여기에는 두가지 방법이 있다. 하나는 $X$ 기저에서 (혹은 $P$ 기저에서) 멱급수를 이용하여 미분방정식을 푸는 것이고 다른 하나는 아주 독특한 사다리 연산자(ladder operator) 를 도입하는 것이다. 여기서는 후자를 먼저 알아보도록 한다.
...
$$ {#eq-QM_ladder_operator_a_dagger}
...
\end{aligned}
$$ {#eq-QM_commutator_for_ladder_and_hamiltonian}
...
\end{aligned}
$$ {#eq-QM_commutator_for_ladder_and_hamiltonian_2}
...
$$ {#eq-QM_property_of_ladder_operator}
...
\end{aligned}
$$ {#eq-QM_property_of_ladder_operator_2}
...
$$ {#eq-QM_energy_eigenstate_from_ground_state_in_sho}
...
사다리 연산자는 $X,\,P$ 에 대해 @eq-QM_ladder_operator_a 와 @eq-QM_ladder_operator_a_dagger 로 정의되었다. 그렇다면 $X,\,P$ 를 사다리 연산자에 대해 다음과 같이 표현 할 수 있다. 
...
에렌페스트 정리를 이용한다. @eq-QM_commutator_for_ladder_and_hamiltonian 로부터
$$
...
이제 사다리 연산자를 이용한 에너지 기저에서 $X$ 기저에 대한 표현으로 변환시키는 것을 알아보자. 우리는 $a|0\rangle = 0$ 임을 안다. 또한 @eq-QM_ladder_operator_a 와 @eq-QM_ladder_operator_a_dagger  에서 보았듯이
...
임을 안다.(@eq-QM_energy_eigenstate_from_ground_state_in_sho) 이로부터 $\Psi_n(x) = \langle x|n\rangle$ 을 얻는다.
...
$$
H_n(\xi) = e^{\xi^2/2}\left(\xi - \dfrac{d}{d\xi}\right)^n e^{-\xi^2/2}
$$ {#eq-QM_generator_of_hermite_polynomial}
...
에르미트 다항식에 대해 알아보자. 우선 @eq-QM_generator_of_hermite_polynomial 로 정의된 에르미트 다항식의 첫 4 함수를 정의대로 계산해보면 다음과 같다.(여기서는 변수를 $\xi$ 에서 $y$ 로 변경한다.)
...
$$
H'_n(y) = 2nH_{n-1}(y).
$$  {#eq-QM_hermite_polynomial_recurssion_relation_1}


또한 @eq-QM_generator_of_hermite_polynomial 로부터 다음의 결과를 얻을 수 있다.
$$
\begin{aligned}
...
$$ {#eq-QM_hermite_polynomial_recurssion_relation_2}


그리고 @eq-QM_hermite_polynomial_recurssion_relation_1 과 @eq-QM_hermite_polynomial_recurssion_relation_2 로 부터 다음의 점화식을 얻을 수 있다.

$$
H_{n+1}(y) = 2yH_n(y) - 2nH_{n-1}(y).
$$ {#eq-QM_hermite_polynomial_recurssion_relation_3}
...
$$ {#eq-QM_orthogonality_of_hermite_polynomials}
...
$x=0$ 에서의 경계조건은 $\Psi(0) = 0$ 이다. @eq-QM_sho_hermite_polynomial_first_4 과 @eq-QM_hermite_polynomial_recurssion_relation_3 로부터 에르미트 다항식의 유용한 성질을 얻을 수 있다. $H_0(y)$ 부터 $H_3(y)$ 까지 보면 $H_n(y)$ 에서 $n$ 이 짝수이면 짝함수(even function), $n$ 이 홀수 이면 홀함수 인 것 같다. 이제 @eq-QM_hermite_polynomial_recurssion_relation_3 를 이용하여 보이자. $n$ 보다 작거나 같은 정수에 대해 이 명제가 성립한다고 가정하자. @eq-QM_hermite_polynomial_recurssion_relation_3 로부터
...
이제 $H_{n}(y)$ 의 상수항을 생각하자. $n$ 이 홀수이면 홀함수 이므로 상수항이 $0$ 이다. 역시 @eq-QM_hermite_polynomial_recurssion_relation_3 에서
