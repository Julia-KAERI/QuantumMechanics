...
이제 @eq-QM_eigenfunction_of_lz_1 의 $R(\rho)$ 에 대해 생각해보자. 파동함수가 $R(\rho) e^{i\phi l_z/\hbar}$ 의 선형결합이라는 것은 $L_z$ 의 고유벡터가 힐베르트 공간 전체를 span 하지 못한다는 것을 의미한다. 즉 $L_z$ 고유공간은 힐베르트 공간의 부분공간이다. $L_z$ 의 고유값 $m\hbar$ 에 대한 파동함수 $\Phi_m(\phi)$ 에 대한 정규화 조건은 
...
라고 놓을 수 있다. 이 식을 @eq-QM_shankar_12_3_7_4 의 조변에 넣으면
