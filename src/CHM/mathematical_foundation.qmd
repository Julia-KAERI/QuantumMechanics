...
1. 벡터는 $|\phi\rangle$ 과 같이 표기하며 **켓(ket)** 라고 한다. $|\phi\rangle$ 에 대한 스칼라 $c$ 곱은 $c|\phi\rangle$ 과 같이 표기한다. 선형결합은 $c_1|\phi_1\rangle + c_2|\phi_2\rangle + \cdots$ 와 같이 표현한다.

2. 연산자 $A$ 에 대한 $|\phi\rangle$ 의 선형변환은 $A|\phi\rangle$ 로 표기한다. 
  
3. $|\phi \rangle$ 에 대한 dual vector 를 $\langle \phi|$ 로 표기하며 **브라(bra)** 이라고 한다. $\langle \phi| = | \phi\rangle^\dagger$ 로 쓰기도 한다. 벡터 $|\phi\rangle$ 과 $|\psi\rangle$ 에 대한 내적은 $\langle \psi | \phi \rangle$ 로 표기한다. 브라와 켓을 합쳐서 **브라켓(bracket)** 이라고 한다. 내적에 대해 $\langle \phi |\psi \rangle = \overline{\langle \psi|\phi \rangle}$ 이다. 
