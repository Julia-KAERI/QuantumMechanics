...
3. 연산자인 $|\phi\rangle\langle \psi|$ 와 내적인 $\langle \psi|\phi\rangle$ 의 구분이 명확해진다. 벡터를 사용한다면 $\boldsymbol{\psi} \boldsymbol{\phi}^\ast$ 와 $\boldsymbol{\psi}^\ast\boldsymbol{\phi}$ 와 같이 구분하기 힘들다. 
...
$$
\langle \psi|A|\phi\rangle = \sum_{i, j} d_j^\ast c_i\langle j|A|i\rangle = \sum_{i, j} d_j^\ast A_{ij} c_i = \left([|\psi\rangle]_{\mathcal{B}}\right)^\dagger [A]_{\mathcal{B}} [|\phi\rangle]_{\mathcal{B}}
$$

임을 알 수 있다. 

벡터와 연산자를 특정 기저에 대해 행렬로 표현 할 수 있으며, 이 때 각각 1-1 대응 관계가 존재한다. 이것을 선형대수학의 기본정리라고 부르기도 한다. 

</br>

### 고유값과 고유벡터

힐베르트 공간 $\mathscr{H}$ 에서 정의된 영벡터가 아닌 벡터 $|\phi\rangle$ 과 연산자 $\Lambda$ 에 대해

$$
\Lambda |\phi\rangle = \lambda|\phi\rangle
$$

을 만족하는 복소수 $\lambda$ 가 존재할 때 $|\phi\rangle$ 을 연산자 $\Lambda$ 에 대해 $\lambda$ 의 **고유값(eigenvalue)** 을 갖는 **고유벡터(eigenvector)** 라고 한다. 
...
힐베르트 공간 $\mathscr{H}$ 에서 정의된 연산자 $U$ 가 $U^\dagger U= UU^\dagger=I$ 를 만족할 때 $U$ 를 **유나타리 연산자(unitary operator)** 라고 한다. 즉 $U$ 가 유니타리 연산자 일 때 $U^{-1}=U^\dagger$ 이다. 특별히 행렬의 모든 성분이 실수 일 때 **직교연산자 (orthogonal operator)** 라고 한다. 

우선 유니타리 행렬은 정의상 가역행렬이다. 따라서 유니타리 행렬은 한 기저를 다른 기저로 변환하는 행렬이다. 유니타리 연산자는 아래와 같은 중요한 성질을 가진다.
...
### Hermitian conjugate, 에르미트 연산자

연산자 $A$ 에 대해 $(A|\psi)^\dagger |\phi\rangle = \langle \psi |A^\dagger |\phi\rangle$ 을 만족하는 $A^\dagger$ 는 항상 존재하며 역시 $\mathscr{H}$ 에서의 선형연산자이다. $A$ 에 대해 $A^\dagger$ 를 $A$ 에 대한  Hermitian cojugate 혹은 Hermitian adjoint 라고 하며 다음을 만족한다.
...
$$ {#eq-CHM_hermitian_conjugate}

$A^\dagger = A$ 인 연산자를 **에르미트 연산자(Heritiaan operator)**라고 한다. 양자역학에서 에르미트 연산자는 매우 중요하며 특별히 다룰 가치가 있다. 
