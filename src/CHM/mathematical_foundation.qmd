...
$|i\rangle$, $|j\rangle$ 이 에르미트 연산자 $A$ 의 고유켓일 때 어떤 조건에서 $|i\rangle +|j\rangle$ 이 $A$ 의 고유캣이 되는가?
...
캣 공간이 에르미트 연산자 $A$ 의 고유캣의 집합 $\{|a'\rangle\}$ 로 span 된다고 하자. 그리고 degeneracy 가 없다고 하자.
...
$A,\,H$ 의 동시 고유캣 $|aE\rangle$ 을 생각하자. $A|ae\rangle = a|ae\rangle,\, H|ae\rangle = e|ae\rangle$ 라고 하자. $[B, H]=0$ 이므로
...
이며 이는 $B|ae\rangle$ 이 $H$ 의 고유캣으로 고유값이 $e$ 인 경우이거나 $B|ae\rangle=|\varnothing\rangle$ 임을 의미한다. 임의의 $\{|ae\rangle\}$ 에 대해 성립하므로 후자라면 $B=0$ 이며 이는 $[A,\,B]\ne 0$ 이라는 조건에 위배된다. 따라서 $B|ae\rangle$ 은 고유값 $e$ 를 갖는 $H$ 의 고유캣이다.
