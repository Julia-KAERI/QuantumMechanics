...
힐베르트 공간은 벡터공간이며 따라서 힐베르트 공간의 원소는 벡터이다. 역사적, 실용적 이유로 양자역학에서의 표기법은 보통 수학에서의 표기법과는 다른 브라켓 표기법을 사용하며 대표적으로 벡터는 $|a\rangle$ 과 같이 $|\,\rangle$ 형태로 표기하며, 선형연산자는 $\hat{L}$ 과 같이 문자 위에 $\hat{~}$ 기호를 씌워 표기하거나 그냥$L$ 과 같이 쓰기도 한다. 양자역학에서의 상태는 수학적으로 힐베르트 공간의 벡터이다. 
...
이 성립하도록 내적 기호 $(\, , \,)$ 혹은 $\langle\, , \, \rangle$ 를 정의한다. 그러나 양자역학에 사용하는 브라켓 표기법은 이와는 좀 다른 규칙을 적용한다.
...
양자역학에서 켓벡터는 양자상태를 표현하는 벡터이며 $0$ 이 아닌 $c\in \mathbb{F}$ 에 대해 $c|\psi \rangle$ 과 $|\psi\rangle$ 을 동일하다고 간주한다. 많은 경우 $|\phi\rangle$ 은 $\langle \phi | \phi \rangle =1$ 인 정규화된벡터를 의미하는 경우가 많다. 또한 유한차원 복소벡터공간에서 양자상태를 표현한다면 $|\phi\rangle$ 을 열벡터로, $\langle \psi |$ 를 행벡터로 표현한다. 브라켓 표기법을 사용하는 이유를 요약하면 다음과 같다.
...
2. 연산자와 벡터의 분리 : $\langle\phi | \hat{A} |\psi\rangle$ 형식이기 때문에 연산자의 연산을 브라와 켓 사이의 공간에서 수행 할 수 있는데, 이렇게 하면 수식표기가 좀 더 깔끔해진다.
