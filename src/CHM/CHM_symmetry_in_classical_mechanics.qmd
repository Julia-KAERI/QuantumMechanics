...
$(q,\,p)\mapsto (Q,\,P)$ 정준변환에서 $\omega (q,\,p)= \overline{\omega}(Q,\,P)$ 이므로 위상공간의 좌표계를 알면 되므로 이제 $\overline{\omega}$ 와 같은 표기는 사용하지 않기로 하자. @thm-CHM_symmetry_invariance_of_poisson_bracket_under_cannonical_transformation 에서 $\theta = \mathcal{H}$ 에 대해서도 성립한다. 따라서,

$$
\dot{\omega} = \{\omega \mathcal{H}\}_{q,\,p} = \{\omega,\, \mathcal{H}\}_{Q,\,P}
$$

이다. 즉 운동에너지 혹은 특정 방향에 대한 각운동량의 시간에 대한 변화량은 위상공간의 좌표 변환에 대해 불변이다. 
...
우선 해밀토니안 $\mathcal{H}=\mathcal{H}(\bf{q},\,\bf{p})$ 와 $(\bf{q},\,\bf{p}) \mapsto (\bf{Q},\,\bf{P})$ regular canonical transformation 에 대해 아래의 두 중요한 명제를 보자.
...
::: {.proof}
이 변환이 정준변환이라는 것은 @exr-shankar_2_8_2 에서 보인다. $(\bf{q},\,\bf{p}) \mapsto (\bf{Q},\,\bf{P})$ 변환이 @eq-CHM_symmetry_infinitesimal_transformation  일 때 $\mathcal{H}$ 의 변화를 $\varepsilon$ 에 대한 1 차 항까지 쓰면 
$$
...
$$ {#eq-CHM_symmetry_delta_of_hamiltonian_by_infinitesimal_transform}
...
@eq-CHM_symmetry_delta_of_hamiltonian_by_infinitesimal_transform 은 해밀토니안의 infinitesimal transform 에 대한 변화였지만 임의의 $\omega$ 에 대해서도 같은 방법으로 정의 할 수 있다. 즉,
...
가 된다. 따라서 phase space 에서의 변환 $(Q,\,P) := (q+\varepsilon\{q,\, H\},\, p+ \varepsilon \{p,\, H\})$ 은 infinitesimal time interval $\varepsilon$ 에 대해 $(q,\,p)$ 가 $(Q,\,P)$ 로 위상공간에서 움직이는 active canonical transformation 이다. 이 운동방정식을 적분하여 $(Q,\,P)$ 를 얻을 수 있다. 이제 $g\ne \mathcal{H}$ 인 일반적인 변환을 생각해보자. 즉
...
::: {#prp-CHM_symmetry_canonital_transoformation_and_solution_of_equation_of_motion}
$\mathcal{H}$ 가 regular canonical transformation $(\bf{q},\,\bf{p}) \mapsto (\bf{Q},\,\bf{P})$ 에 대해 불변이며(infinitesimal 일 필요는 없다), $(\bf{q}(t),\,\bf{p}(t))$ 가 운동방정식의 해라면 $(\bf{Q}(t),\, \bf{P}(t))$ 도 운동방정식의 해이다.
:::
